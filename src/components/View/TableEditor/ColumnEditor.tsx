	...
	useEffect(() => {
		...
		if (columnTable && column) {
			...
			if (
				...
				query += `ALTER TABLE ${columnTable.schema}.${columnTable.name} DROP CONSTRAINT ${columnTable.name}_${column.name}_fkey;`;
				query += `ALTER TABLE ${columnTable.schema}.${columnTable.name} ADD CONSTRAINT ${columnTable.name}_${column.name}_fkey REFERENCES ${table.name}(${foreignColumn});`;
			}
			if (column.name !== name) {
				query += `ALTER TABLE ${columnTable.schema}.${columnTable.name} RENAME COLUMN ${column.name} TO ${name};`;
			}
			const _is_array = constraints.includes("ARRAY");
			if (column.type_name !== type) {
				query += `ALTER TABLE ${columnTable.schema}.${columnTable.name} ALTER COLUMN ${name} TYPE ${type}${
					_is_array ? "[]" : ""
				};`;
			}
			if ((column.default_value ?? "") !== defaultVal) {
				query += `ALTER TABLE ${columnTable.schema}.${columnTable.name} ALTER COLUMN ${name} SET DEFAULT '${defaultVal}';`;
			}
			if (parsedConstraints.includes("NOT NULL") && !constraints.includes("NOT NULL")) {
				query += `ALTER TABLE ${columnTable.schema}.${columnTable.name} ALTER COLUMN ${name} DROP NOT NULL;`;
			} else if (!parsedConstraints.includes("NOT NULL") && constraints.includes("NOT NULL")) {
				query += `ALTER TABLE ${columnTable.schema}.${columnTable.name} ALTER COLUMN ${name} SET NOT NULL;`;
			}
			if (parsedConstraints.includes("IDENTITY") && !constraints.includes("IDENTITY")) {
				query += `ALTER TABLE ${columnTable.schema}.${columnTable.name} ALTER COLUMN ${name} DROP IDENTITY;`;
			} else if (!parsedConstraints.includes("IDENTITY") && constraints.includes("IDENTITY")) {
				query += `ALTER TABLE ${columnTable.schema}.${columnTable.name} ALTER COLUMN ${name} ADD GENERATED BY DEFAULT AS IDENTITY;`;
			}
			if (parsedConstraints.includes("PRIMARY KEY") && !constraints.includes("PRIMARY KEY")) {
				query += `ALTER TABLE ${columnTable.schema}.${columnTable.name} DROP CONSTRAINT ${columnTable.name}_pkey;`;
			} else if (!parsedConstraints.includes("PRIMARY KEY") && constraints.includes("PRIMARY KEY")) {
				query += `ALTER TABLE ${columnTable.schema}.${columnTable.name} ADD PRIMARY KEY (${name});`;
			}
			if (parsedConstraints.includes("UNIQUE") && !constraints.includes("UNIQUE")) {
				query += `ALTER TABLE ${columnTable.schema}.${columnTable.name} DROP CONSTRAINT ${columnTable.name}_${column.name}_key;`;
			} else if (!parsedConstraints.includes("UNIQUE") && constraints.includes("UNIQUE")) {
				query += `ALTER TABLE ${columnTable.schema}.${columnTable.name} ADD CONSTRAINT ${columnTable.name}_${column.name}_key UNIQUE (${name});`;
			}
		...
		if (columnTable && name && type) {
			setQuery((prev) => ({
				...prev,
				[idx]: `ALTER TABLE ${columnTable.schema}.${columnTable.name} ADD COLUMN ${_query};`,
			}));
